...
**General/template files** are those files which should in essence not be changed. They are either central to the working of the package or serve as templates for use of the package. These include:
### calculate_coeffs.py
The central program that calculates the coefficients starting from any $W_{eq}$ and $\bar{W}$. This can be used as black box and should never be changed without good reason.
