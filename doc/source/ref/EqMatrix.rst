...
Starting from an empty matrix, interactions can be added by using the :func:`add_trans` method:
...
.. caution::
    ...
    done, the matrix diagonal needs to be recalculated using :func:`calc_diag`. :func:`add_trans` on the other hand
    takes care of this for you.

Calculating eigensystem
-----------------------

After building the system, we can calculate the eigensystem and equilibrium distribution using :func:`calc_eig`. These
values are then returned and stored as attributes of the object. By default, detailed balance is checked.

.. note::
    The method :func:`calc_peq` is basically an alias for :func:`calc_eig`. Because the equilibrium distribution is
...
By default, the EqMatrix is treated as a symbolic matrix. If numeric calculations are preferred, use the :func:`to_num()`
...
convert back to symbolical calculations, use :func:`to_sym`.
